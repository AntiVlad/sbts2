...
export async function GET(req: Request) {
  ...
  if (type === 'facilitators') {
    const rows = await prisma.facilitatorAttendance.findMany({
      where: { date },
      include: { facilitator: true },
      orderBy: [{ clockIn: 'asc' }],
    })
    let csv = 'name,course,date,clock_in,clock_out,duration_minutes\n'
    for (const r of rows) {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="facilitator-attendance-${date}.csv"`,
      },
    })
  } else {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        'Content-Type': 'text/csv; charset=utf-8',
        'Content-Disposition': `attachment; filename="student-attendance-${date}.csv"`,
      },
    })
  }
}
