        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th className="th">
                <a className="underline" href={`?date=${encodeURIComponent(date)}&search=${encodeURIComponent(search)}&page=${page}&sort=${sort === 'asc' ? 'desc' : 'asc'}`}>
                  Clock-In {sort === 'asc' ? '▲' : '▼'}
                </a>
              </th>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-t border-white/10">
                  <td className="td">{r.student.regNo}</td>
                  <td className="td">{r.student.fullName}</td>
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-white/70">
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            href={`?date=${encodeURIComponent(date)}&search=${encodeURIComponent(search)}&page=${page - 1}`}
          >
            Prev
          </a>
          <a
